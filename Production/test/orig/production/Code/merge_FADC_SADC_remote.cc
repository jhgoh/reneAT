...
void merge_FADC_SADC_remote(int run, int maxsubrun, int fadcSubrun, int sadcSubrun, int sadcEvent, unsigned int sadcTrgnum, const char* RAWDir, const char* DataDir){
    ...
    while(1){
        ...
        if(fadcEndSub){
            ...
            inf_fadc = TFile::Open(Form("%s/FADC_%s.root.%s", RAWDir, run_str, fadcsubrun_str), "read");
        ...
        if(sadcEndSub){
            ...
            inf_sadc = TFile::Open(Form("%s/SADC_%s.root.%s", RAWDir, run_str, sadcsubrun_str), "read");
